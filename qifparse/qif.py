...
class Qif(object):
    ...
    def add_prices(self, item):
        if not isinstance(item, Prices):
            raise RuntimeError(six.u("item not recognized"))
        self._prices.append(item)
    ...
    def get_securities(self, name=None, atype=None):
        res = []
        return tuple(res)

    def get_prices(self, name=None, atype=None):
        if not name and not atype:
...
class Security(BaseEntry):
    _fields = [
        ...
        Field('security', 'string', 'S'),
        Field('type', 'string', 'T'),
    ]

class Prices(BaseEntry):
    _fields = [
        Field('name', 'string', 'N', required=True),
    ]
