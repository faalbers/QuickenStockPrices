...
class QifParser(object):
    ...
    def parse(cls_, file_handle, date_format=None):
        ...
        qif_obj = Qif()
        chunks = qif_obj.get_chunks(data)
        last_type = None
        last_account = None
        ...
        for chunk in chunks:
            ...
            while first_line in OPTION_TYPES:
                first_line_idx += 1
                first_line = chunk.split('\n')[first_line_idx]
            ...
            elif last_type == 'prices':
                qif_obj.add_prices(item)
        return qif_obj
    ...
    @classmethod
    def parsePrices(cls_, chunk):
        """ Parsing Prices Entries
        """
        curItem = Prices()
        lines = chunk.split('\n')
        for line in lines:
            if not len(line) or line[0] == '\n' or \
                    line.startswith('!Type:Prices'):
                continue
            elif line[0] == '"':
                data = line.split(',')
                curItem.name = data[0].strip('"')
        return curItem
